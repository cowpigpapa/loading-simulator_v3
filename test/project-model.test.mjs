...
test('legacy snapshots migrate and current metadata is preserved',()=>{
  ...
  assert.equal(current.algorithmVersion,'extreme-dblf-transport-stability-2026.08');assert.equal(current.resultSummary.loaded,36);assert.equal(current.resultSummary.totalWeight,6692);
});
