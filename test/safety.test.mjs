...
test("all supported container sizes preserve packing invariants", () => { context.caseItems=Array.from({length:18},(_,i)=>({...base,l:700+(i%3)*180,w:500+(i%2)*220,h:450+(i%4)*120,weight:90+i*8,rotate:true,unit:i+1,volume:(700+(i%3)*180)*(500+(i%2)*220)*(450+(i%4)*120)}));for(const key of ['20ft','40ft','40hc','45hc']){context.caseKey=key;const packed=run(`packPlan(CONTAINERS[caseKey],caseItems,'hybrid')`);context.packedResult=packed;const validation=run(`LoadwiseValidator.validateShipment({priority:'hybrid',containers:[packedResult],unallocated:packedResult.rejected,totalUnits:caseItems.length})`);assert.equal(validation.valid,true,key);assert.equal(validation.metrics.loaded+validation.metrics.unallocated,18,key)} });
...
test("balance strategy keeps a uniform full load in one container and centers it", () => { context.caseItems=Array.from({length:36},(_,i)=>({...base,rotate:true,unit:i+1}));const packed=run(`packPlan(CONTAINERS['20ft'],caseItems,'balance')`),c=packed.container,total=packed.placed.reduce((sum,p)=>sum+p.weight,0),x=packed.placed.reduce((sum,p)=>sum+(p.x+p.l/2)*p.weight,0)/total,y=packed.placed.reduce((sum,p)=>sum+(p.y+p.w/2)*p.weight,0)/total;assert.equal(packed.rejected.length,0);assert.equal(packed.placed.length,36);assert.ok(Math.abs(x/c.l-.5)<=.05);assert.ok(Math.abs(y/c.w-.5)<=.05) });
test("safe-width hybrid falls back before unnecessarily adding a container", () => { context.caseItems=[...Array.from({length:12},(_,i)=>({...base,name:'wide',w:820,rotate:true,unit:i+1})),...Array.from({length:12},(_,i)=>({...base,name:'medium',w:700,rotate:true,unit:i+13})),...Array.from({length:16},(_,i)=>({...base,name:'narrow',w:530,rotate:true,unit:i+25}))];const packed=run(`packPlan(CONTAINERS['20ft'],caseItems,'hybrid')`);assert.equal(packed.rejected.length,0);assert.equal(packed.placed.length,40) });
test("automatic comparison prioritizes fewer unallocated items", () => { context.LoadwiseInsights={balance:()=>({xOffset:0,yOffset:0})};context.caseBase=base;run(`casePlans=[{priority:'sequence',loads:[{container:CONTAINERS['20ft'],placed:[],rejected:[]}],remaining:[caseBase]},{priority:'hybrid',loads:[{container:CONTAINERS['20ft'],placed:[],rejected:[]}],remaining:[]}]`);assert.equal(run(`selectBestStrategy(casePlans).priority`),'hybrid') });
test("automatic comparison prefers the safer CTU distribution after shipment completeness", () => { context.LoadwiseInsights={balance:load=>({xOffset:load.risk,yOffset:0}),ctu:load=>({xOffset:load.risk,yOffset:0,concentration:load.concentration,vertical:20,level:load.risk>10?'danger':'safe'})};run(`casePlans=[{priority:'sequence',loads:[{risk:14,concentration:80,placed:[]}],remaining:[]},{priority:'hybrid',loads:[{risk:2,concentration:55,placed:[]}],remaining:[]}]`);assert.equal(run(`selectBestStrategy(casePlans).priority`),'hybrid') });
...
test("safe-width hybrid chooses the smallest short-side residual before used length", () => { context.caseItem={...base,l:900,w:500,h:400,rotate:true};context.casePlaced=[{...base,x:0,y:0,z:0,l:900,w:1300,h:400}];run(`preparePackingWidths([caseItem])`);const best=run(`findExtremePlacement({l:5000,w:2352,h:2393,maxWeight:28200},caseItem,casePlaced,'hybrid')`);assert.equal(best.s.x,0);assert.equal(best.s.y,1300);assert.equal(best.d[1],900) });
test("projected width combinations use packing input instead of UI state", () => { context.caseItems=[{...base,l:700,w:700},{...base,l:952,w:952}];run(`products=[];preparePackingWidths(caseItems)`);assert.equal(run(`projectedWidthGap(1652)`),0) });
