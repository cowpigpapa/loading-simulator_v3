...
test("width-based strategies fall back before unnecessarily adding a container", () => { context.caseItems=[...Array.from({length:12},(_,i)=>({...base,name:'wide',w:820,rotate:true,unit:i+1})),...Array.from({length:12},(_,i)=>({...base,name:'medium',w:700,rotate:true,unit:i+13})),...Array.from({length:16},(_,i)=>({...base,name:'narrow',w:530,rotate:true,unit:i+25}))];for(const strategy of ['width','hybrid']){context.caseStrategy=strategy;const packed=run(`packPlan(CONTAINERS['20ft'],caseItems,caseStrategy)`);assert.equal(packed.rejected.length,0);assert.equal(packed.placed.length,40)} });
