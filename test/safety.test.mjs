...
test("50 percent rear support is accepted", () => { context.sideCargo=[{x:0,y:1000,z:0,l:1000,w:400,h:1100}];assert.equal(run(`lateralSupportAxes({x:1000,y:1000,z:0},[800,800,2200],sideCargo,CONTAINERS['20ft']).x`),true) });
test("unbraced slender cargo gets a lashing band", () => { context.bandLoad={container:{...run(`CONTAINERS['20ft']`)},placed:[{...base,x:1800,y:700,z:0,l:800,w:800,h:2200,name:'tall'}]};const plan=run(`buildSecuringPlan(bandLoad)`);assert.equal(plan.bands.length,1);assert.match(plan.bands[0].location,/밴드/) });
