...
test("second-tier cargo uses a band when one or two directions lack 50 percent support", () => { const c=run(`CONTAINERS['20ft']`);context.bandLoad={container:{...c},placed:[{...base,x:c.l-800,y:0,z:0,l:800,w:800,h:800,name:'base'},{...base,x:c.l-800,y:0,z:800,l:800,w:800,h:800,name:'upper'}]};const plan=run(`buildSecuringPlan(bandLoad)`);assert.equal(plan.bands.length,1);assert.equal(plan.bands[0].product,'upper');assert.match(plan.bands[0].location,/2단 이상/) });
test("three unsupported directions require rearrangement instead of a band", () => { const c=run(`CONTAINERS['20ft']`);context.bandLoad={container:{...c},placed:[{...base,x:1000,y:0,z:0,l:800,w:800,h:800,name:'base'},{...base,x:1000,y:0,z:800,l:800,w:800,h:800,name:'upper'}]};const plan=run(`buildSecuringPlan(bandLoad)`);assert.equal(plan.bands.length,0);assert.equal(plan.bandWarnings.length,1);assert.match(plan.bandWarnings[0].location,/3방향/) });
